<?php
  ...
  function donation_data_us($start_date, $end_date) {
    ...
    $sql .= "ROUND((SUM(DG5.DONATION.AMOUNT) / (SELECT SUM(ELEHMANN.STATE.POPULATION) FROM ELEHMANN.STATE)), 4) AS DONATIONS_PER_CAPITA ";
    $sql .= "FROM DG5.DONATION ";
    $sql .= "JOIN ELEHMANN.COMMITTEE ON ELEHMANN.COMMITTEE.COMMITTEE_ID = DG5.DONATION.COMMITTEEID ";
  ...
  function donation_data_state($start_date, $end_date, $state) {
    global $db;
    ...
    $sql .= "COUNT(DG5.DONATION.AMOUNT) AS NUM_DONATIONS, ";
    $sql .= "ROUND(AVG(DG5.DONATION.AMOUNT), 2) AS DONATION_SIZE, ";
    $sql .= "ROUND((SUM(DG5.DONATION.AMOUNT) / (SELECT ELEHMANN.STATE.POPULATION FROM ELEHMANN.STATE WHERE ELEHMANN.STATE.CODE = :state_bv)), 4) AS DONATIONS_PER_CAPITA ";
    $sql .= "FROM DG5.DONATION ";
    $sql .= "JOIN ELEHMANN.COMMITTEE ON ELEHMANN.COMMITTEE.COMMITTEE_ID = DG5.DONATION.COMMITTEEID ";
    $sql .= "WHERE DG5.DONATION.DAY >= :start_date_bv AND DG5.DONATION.DAY <= :end_date_bv ";
    $sql .= "AND DG5.DONATION.STATE = :state_bv ";
    $sql .= "GROUP BY ELEHMANN.COMMITTEE.CANDIDATE ";
    $sql .= "ORDER BY ELEHMANN.COMMITTEE.CANDIDATE ASC";
    //echo $sql;
    $query = oci_parse($db, $sql);
  ...
  function donation_data_city($start_date, $end_date, $state, $city) {
    ...
    $sql .= "AND REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(ELEHMANN.CITY.CITY, ' city and burrow$', ''),' city$',''),' borough$',''),' town$',''),' municipality$',''),' village$',''),' unified government$','') LIKE :ecity_bv)), 4) AS DONATIONS_PER_CAPITA ";
    $sql .= "FROM DG5.DONATION ";
    $sql .= "JOIN ELEHMANN.COMMITTEE ON ELEHMANN.COMMITTEE.COMMITTEE_ID = DG5.DONATION.COMMITTEEID ";
    $sql .= "WHERE DG5.DONATION.DAY >= :start_date_bv AND DG5.DONATION.DAY <= :end_date_bv ";
